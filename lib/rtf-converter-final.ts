...
class RtfParser {
  ...
  private decodeWindows1256(code: number): string {
    ...
    if (cp1256Map[code]) return cp1256Map[code];
   ...
   */
  private handleControlSymbol(symbol: string): void {
    switch (symbol) {
      case '~': this.html += '&nbsp;'; break;
      case '-': this.html += '&shy;'; break;
      case '_': this.html += '‑'; break;
    }
  }
   ...
   */
  private handleControlWord(word: string, param: number | null): void {
    // Unicode
    ...
    // Alignment
    if (word === 'qr') { this.state.alignment = 'right'; return; }
    if (word === 'ql') { this.state.alignment = 'left'; return; }
    if (word === 'qc') { this.state.alignment = 'center'; return; }
    if (word === 'qj') { this.state.alignment = 'justify'; return; }
    
    // RTL/LTR
    if (word === 'rtlpar' || word === 'rtlch') { this.state.isRtl = true; return; }
    if (word === 'ltrpar' || word === 'ltrch') { this.state.isRtl = false; return; }
    
    // Special characters
    if (word === 'bullet') { this.html += '•'; return; }
    if (word === 'tab') { this.html += '&nbsp;&nbsp;&nbsp;&nbsp;'; return; }
    if (word === 'emdash') { this.html += '—'; return; }
    if (word === 'endash') { this.html += '–'; return; }
    if (word === 'lquote') { this.html += '\u2018'; return; }
    if (word === 'rquote') { this.html += '\u2019'; return; }
    if (word === 'ldblquote') { this.html += '\u201C'; return; }
    if (word === 'rdblquote') { this.html += '\u201D'; return; }
  }

  /**
   * Sync کردن HTML با تغییرات state (وقتی از group خارج میشیم)
   */
  private syncHtmlWithStateChange(oldState: FormattingState, newState: FormattingState): void {
    // Bold
   ...
   */
  private closeAllOpenTags(): void {
    if (this.state.backgroundColor !== null) this.html += '</span>';
    if (this.state.color !== null) this.html += '</span>';
    if (this.state.subscript) this.html += '</sub>';
    if (this.state.superscript) this.html += '</sup>';
    if (this.state.strike) this.html += '</s>';
    if (this.state.underline) this.html += '</u>';
    if (this.state.italic) this.html += '</i>';
    if (this.state.bold) this.html += '</b>';
  }
