...
export function rtfToHtml(rtf: string): string {
  ...
  let html = '';
  let openTags: string[] = []; // برای track کردن تگ‌های باز
  let i = 0;
...
function parseFontTable(rtf: string): Map<number, FontInfo> {
  const fonts = new Map<number, FontInfo>();
  const fontTableMatch = rtf.match(/\{\\fonttbl([^}]+(?:\{[^}]*\}[^}]*)*)\}/);
  
  if (!fontTableMatch) return fonts;
...
function parseColorTable(rtf: string): ColorInfo[] {
  const colors: ColorInfo[] = [];
  const colorTableMatch = rtf.match(/\{\\colortbl\s*;([^}]+)\}/);
  
  if (!colorTableMatch) return colors;
...
function extractMainContent(rtf: string): string {
...
function parseControlWord(content: string, startIndex: number): { word: string; param: number | null; nextIndex: number } {
...
function closeInlineTags(openTags: string[]): string {
  let html = '';
  ...
  for (let i = openTags.length - 1; i >= 0; i--) {
    ...
    else if (tag === 'color') html += '</span>';
    else if (tag === 'background') html += '</span>';
  }
  return html;
}

function reopenInlineTags(
  state: FormattingState,
  openTags: string[],
  fonts: Map<number, FontInfo>,
  colors: ColorInfo[]
...
function closeBold(openTags: string[]): string {
...
function closeItalic(openTags: string[]): string {
...
function closeUnderline(openTags: string[]): string {
...
function closeStrike(openTags: string[]): string {
...
function closeSuperscript(openTags: string[]): string {
...
function closeSubscript(openTags: string[]): string {
...
function closeColorSpan(openTags: string[]): string {
...
function closeBackgroundSpan(openTags: string[]): string {
