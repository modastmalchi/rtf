...
function rtfToHtml(rtf) {
  ...
  // Cross-runtime byte decoder for single bytes according to code page
  const textDecoders = new Map();
  let iconvDecode = null;
  try {
    if (typeof require === 'function') {
      const iconv = require('iconv-lite');
      if (iconv && typeof iconv.decode === 'function') iconvDecode = iconv.decode;
    }
  } catch (_) { /* ignore */ }
  function decodeByteWithEncoding(byte, encoding) {
    // Prefer iconv-lite in Node (covers many legacy encodings reliably)
    if (iconvDecode) {
      try {
        return iconvDecode(Buffer.from([byte]), encoding);
      } catch (_) { /* continue to TD fallback */ }
    }
    // Browser/Node WHATWG TextDecoder path
    try {
      let TD = (typeof TextDecoder !== 'undefined') ? TextDecoder : (typeof require === 'function' ? require('util').TextDecoder : null);
      if (TD) {
        if (!textDecoders.has(encoding)) {
          textDecoders.set(encoding, new TD(encoding, { fatal: false }));
        }
...
function htmlToRtf(html) {
  ...
  if (typeof DOMParser !== 'undefined') {
    ...
    // Node.js - simple parsing
    // For simplicity, we'll do basic tag parsing
    return htmlToRtfSimple(html);
  }
...
function htmlToRtfSimple(html) {
  // Simple HTML to RTF for Node.js without DOM parser
  // This is a basic implementation that handles common tags
  
  let rtf = '{\\rtf1\\ansi\\ansicpg1256\\deff0\n';
  rtf += '{\\fonttbl{\\f0\\fnil\\fcharset178 Arial;}}\n';
